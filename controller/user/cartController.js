  ...
  try {
    ...
    if (user.isBlocked == true) {
      return res
        ...
        .json({ success: false, error: "You have no access" });
    }
    ...
    if (!sizeObject) {
      return res
        .status(statusCodes.BAD_REQUEST)
        .json({ success: false, error: "size is not found for this product" });
    }
    ...
    if (existingItemIndex > -1) {
      ...
      if (existingItem.quantity + 1 > stock) {
        return res
          .status(statusCodes.BAD_REQUEST)
          .json({ success: false, error: "Quantity exceeds available stock" });
      }
  ...
  try {
    ...
    for (const item of cart.items) {
      ...
      if (typeof regularPrice !== "number" || typeof salePrice !== "number") {
        return res
          .status(statusCodes.BAD_REQUEST)
          .json({ success: false, error: "Invalid price data for product" });
      }
      ...
      if (!sizeStock || sizeStock.stock < quantity) {
        return res
          ...
          .json({
            ...
            } (${size}) has insufficient stock. Only ${
    ...
    if (paymentMethod === "wallet") {
      ...
      if (!wallet || wallet.balance < totalAmount) {
        return res
          ...
          .json({
            success: false,
            error: "your wallet has insufficient balance",
          });
      }
    ...
    res.json({
      success: true,
      error: "Order updated successfully",
      cartEmpty: true,
    ...
    for (let item of updateOrder.items) {
      ...
      if (product) {
        ...
        if (sizeStock !== -1) {
          ...
          console.log(
            `Updated stock for size ${item.size} by ${item.quantity}`
          );
        }
